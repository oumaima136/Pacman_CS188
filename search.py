...
def uniformCostSearch(problem):
    ...
    while not queue.isEmpty():
        ...
        if currentNode not in visited:
            ...
            for node,action,c in problem.getSuccessors(currentNode):
                act = actions + [action]
                cost = problem.getCostOfActions(act)
                queue.push((node,act),cost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not queue.isEmpty():
        ...
        if currentNode not in visited:
            ...
            for node,action,c in problem.getSuccessors(currentNode):
                act = actions + [action]
                cost = problem.getCostOfActions(act)+ heuristic(node,problem)
                queue.push((node,act),cost)
    util.raiseNotDefined()
